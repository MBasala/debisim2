...
class DEBISimPipeline(object):
    ...
    def __init__(
        ...
        # .tar.zst (preferred) or .tar.gz file via a dedicated writer
        # thread (and a small serializer thread pool).  Data passes by
        # reference between the main thread and the workers; zstd/zlib
        # release the GIL during compression so the compute pipeline
        # isn't blocked.  Per-file FITS compression is disabled — the
        # outer archive layer handles compression once.
